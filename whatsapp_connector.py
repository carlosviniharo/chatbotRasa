...
"""
This module defines a customized input and output channel for integrating with the 
WhatsApp Business Cloud API, tailored specifically for the Asistec project. 

Since the Rasa framework does not provide a built-in implementation for the 
WhatsApp Business Cloud API, this module fills that gap by defining custom classes 
that enable communication via WhatsApp.

### Notes
- This is the initial version of the integration. Additional methods and improvements 
  may be required to fully support all features of the WhatsApp Cloud API.
- The implementation uses the Sanic web framework to handle HTTP requests from WhatsApp.
